...
# create node and descendant node hash
# TODO: Maybe combine this with the node_genes loop?
...
foreach my $id (keys %annotation){
    ...
    my ($annotation_id, $an, $ptn, $go, $go_name, $type, $date)=split(/\;/, $line);
    my %allDescendants = &findDescendantInPTN($an, \%parent_child, \%leaf, \%an_ptn); # all gene in current book.
    foreach my $descendant (keys %allDescendants){
        $node_descendants{$ptn}{$descendant}=1;
    }
}
...
foreach my $annotation_id (keys %annotation){
    ...
    if (defined $nots{$annotation_id}){
        foreach $not_annotation_id (keys %{$nots{$annotation_id}}){
            ...
            if (defined $node_descendants{$ptn_n}){
                foreach my $gene (keys %{$node_descendants{$ptn_n}}){
...
sub findDescendantInPTN{
    my ($an, $parent_child_href, $leaf_href, $an_ptn)=@_;
